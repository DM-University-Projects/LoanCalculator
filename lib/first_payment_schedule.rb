module FirstPaymentSchedule
  def first_payment_schedule(principal_amount:, interest_rate:, monthly:, request_date:)
    @monthly = monthly
    @request_date = request_date
    @principal_amount = principal_amount
    @amortization_schedule = []
    @interest_per_period = interest_per_period(interest_rate)
    @monthly_payment = emi(principal_amount, monthly)
    amortization_schedule
  end

  def emi(principal_amount,monthly)
    (( principal_amount * @interest_per_period ) * (( @interest_per_period + 1 )**monthly)) /
      ((( @interest_per_period + 1 )**monthly ) - 1 )
  end

...
RSpec.describe EqualPaymentsSchedule do
  describe 'EqualPaymentsSchedule' do
    let(:dummy_class) { Class.new }
    before do
      dummy_class.extend(EqualPaymentsSchedule)
    end

    it 'returns the amortization schedule for the loan' do
      loan = build(:loan, loan_amount: 10_000, monthly: 12, interest_rate: 10, request_date: Date.new(2017, 0o5, 15))
      amortization_schedule = dummy_class.equal_payments_schedule(principal_amount: loan.loan_amount,
                                                                                           term: loan.term,
